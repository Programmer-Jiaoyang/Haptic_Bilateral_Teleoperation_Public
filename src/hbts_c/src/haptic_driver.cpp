...
public:
    ...
    {   
        ...
        this->declare_parameter("simulate_button", true);
        this->get_parameter("simulate_button", simulate_button_); 
        ...
        button_pub_ = this->create_publisher<std_msgs::msg::Float32>("/haptic_button", 10);
...
private:
    ...
    rclcpp::Publisher<std_msgs::msg::Float32>::SharedPtr button_pub_;
    ...
    void update_loop()
    {
        ...
        position_msg.data = {float(px), float(py), float(pz)};
        orientation_msg.data = {float(roll), float(pitch), float(yaw)};
        ...
        ft_msg.data = {float(fy), float(fx), float(-fz)};
        ft_pub_->publish(ft_msg);

        // Publish button
        std_msgs::msg::Float32 button_msg; 
        if (simulate_button_)
        {
            int button_state = dhdGetButton(0);
            if (button_state < 0)
                return;
            button_msg.data = (button_state != 0) ? 1.0f : 0.0f;
        }
        else
        {   
            double Deg = 0.0;

            // int button_state = dhdGetGripperAngleDeg(&Deg);
            int button_state = dhdGetGripperGap(&Deg);
    ...
    {   
        ...
        dhdSetForceAndTorqueAndGripperForce(fy, fx, -fz, 0.0, 0.0, 0.0, 0.0);
    }
    else
    {
